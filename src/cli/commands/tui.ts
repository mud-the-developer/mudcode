    ...
    if (command.startsWith('/new')) {
      try {
        ...
        if (installed.length === 0) {
          append('⚠️ No agent CLIs found. Install one first (claude, gemini, opencode, codex).');
          return false;
        }
        ...
        if (!selected) {
          append(`⚠️ Unknown agent '${parsed.agentName}'. Try claude, gemini, opencode, or codex.`);
          return false;
        }
